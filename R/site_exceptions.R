site_exceptions <- function(site_code, var_data, att_data, qc_val) {
  ...
  if (site_code == "AU-Cow") {
    ...
    lwdown <- FluxnetLSM:::SynthesizeLWdown(TairK = var_data$Tair[ind_start:ind_end],
    ...
    rh <- FluxnetLSM:::VPD2RelHum(VPD=var_data$VPD, airtemp=var_data$Tair,
    ...
    lwdown <- FluxnetLSM:::SynthesizeLWdown(TairK = var_data$Tair[inds],
                               RH = var_data$RH[inds])
